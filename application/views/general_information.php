        ...
        <div id="wrap"><input type="hidden" id="base_url" value="<?php echo (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" . trim($_SERVER['SERVER_NAME']) : "http://" . trim($_SERVER['SERVER_NAME']); ?>">
            <div id="main" class="container" style="padding-bottom:0px;"> <!--  -->
                ...
                <?php 
                    ...
                    <div class="row">
                        <div class="col-sm-9" style="padding:10px;padding-right:0px;">
                            <div class="panel panel-default" style="min-height:100%;width:100%;float:right;">
                                <div id="informationContainer" class="panel-body" style="width:100%;">
                                    <?php 
                                        $cnt = 0;
                                        $gen_info_count = count( $general_information );
                                        while( $cnt < $gen_info_count ){ ?>
                                            <div class="row">
                                                <div class="col-sm-6" id="info_elem_<?php echo trim($general_information[$cnt]['notification_id']);?>">
                                                    <div class="panel panel-default panel-custom"
                                                         id="info_<?php echo trim($general_information[$cnt]['notification_id']);?>" 
                                                            onmouseover="addPanelHighLight('info_' + '<?php echo trim($general_information[$cnt]['notification_id']);?>');" 
                                                            onmouseout="removePanelHighlight('info_' + '<?php echo trim($general_information[$cnt]['notification_id']);?>');"
                                                            data-toggle="modal" data-target="#informationModal">
                                                        <div class="panel-heading info-head">
                                                            <p><?php echo trim($general_information[$cnt]['notification_heading']); ?></p>
                                                            <input type="hidden" 
                                                                   id="notif_head_<?php echo trim($general_information[$cnt]['notification_id']);?>"
                                                                   value="<?php echo trim($general_information[$cnt]['notification_heading']); ?>" >
                                                        </div>
                                                        <div class="panel-body">
                                                            <p><?php echo substr( trim($general_information[$cnt]['notification_text']), 0, 150 ) . " ... "; ?></p>
                                                            <input type="hidden" id="notif_text_<?php echo trim($general_information[$cnt]['notification_id']);?>"
                                                                           value="<?php echo trim($general_information[$cnt]['notification_text']); ?>" >
                                                            <input type="hidden" id="img_url_<?php echo trim($general_information[$cnt]['notification_id']);?>"
                                                                           value="<?php echo ((!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" . trim($_SERVER['SERVER_NAME']) : "http://" . trim($_SERVER['SERVER_NAME'])) . _IMAGE_URL . "/" . _IMAGE_SCHOOL_NOTIFICATION_NUM 
                                                                                   . "/" . trim($general_information[$cnt]['notification_image_name']); ?>" >
                                                        </div>
                                                    </div>
                                                </div>
                                            <?php $cnt++;
                                                if( $cnt < $gen_info_count ){ ?>
                                                <div class="col-sm-6" id="info_elem_<?php echo trim($general_information[$cnt]['notification_id']);?>" >
                                                    <div class="panel panel-default panel-custom" 
                                                         id="info_<?php echo trim($general_information[$cnt]['notification_id']);?>" 
                                                            onmouseover="addPanelHighLight('info_' + '<?php echo trim($general_information[$cnt]['notification_id']);?>');" 
                 ...
                 <div id="informationModal" class="modal fade" role="dialog" style="height:100%;">
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal">&times;<small>CLOSE</small></button>
                                <h4 class="modal-title" id="informationTitle"><strong></strong></h4>
                            </div>
                            <div class="modal-body" id="informationContent">
                                <!--<p>Hello there, &nbsp;&nbsp;&nbsp;   asdf<br> How are you doing? <br> This is a wonderful day!</p>-->
                                <img id="informationContentImage" class="img-rounded img-responsive" style="padding-bottom:20px;" alt="">
                                <p id="informationContentText" style="width:100%;word-wrap:break-word;"></p>
                            </div>
                        </div>
                    </div>
                 </div>
